      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm flex items-center gap-4">
          ...
          <div>
            ...
            <h3 className="text-2xl font-bold text-slate-800 mt-1">{stats.salesToday}</h3>
          </div>
        </div>
        ...
        <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm flex items-center gap-4">
          ...
          <div>
            ...
            <h3 className="text-2xl font-bold text-slate-800 mt-1">{stats.desyncCount}</h3>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 bg-white p-6 rounded-xl border border-slate-200 shadow-sm flex flex-col justify-between">
          <div className="flex items-center justify-between mb-6">
            <div>
              <h3 className="font-semibold text-slate-800">Rendimiento de Ventas</h3>
              <p className="text-xs text-slate-400 mt-1">Unidades vendidas en los últimos 7 días</p>
            </div>
            <span className="flex items-center gap-1 text-xs font-bold text-green-600 bg-green-50 px-2 py-0.5 rounded font-mono">
              <TrendingUp size={12} /> Live
            </span>
          </div>

          <div className="h-56 w-full flex items-end justify-between px-4 pb-2 border-b border-slate-100">
            {chartData.map((d, i) => {
              const pct = (d.value / maxVal) * 100;
              return (
                <div key={i} className="flex flex-col items-center gap-2 group cursor-pointer relative" style={{ width: '12%' }}>
                  <div className="absolute bottom-full mb-2 bg-slate-800 text-white text-[10px] font-bold py-1 px-2 rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap shadow-md z-10">
                    {d.value} uds.
                  </div>
        ...
        <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm flex flex-col justify-between">
          <div>
            <h3 className="font-semibold text-slate-800 mb-4">Alertas Críticas</h3>
              ...
              {quickAlerts.length === 0 ? (
                ...
                quickAlerts.slice(0, 3).map((alert, index) => (
                  <div key={index} className="flex items-start gap-3 p-3 rounded-lg bg-slate-50 border border-slate-100">
                    <div className={`mt-0.5 p-1 rounded ${alert.severity === 'HIGH' ? 'bg-red-50 text-red-600' : 'bg-amber-50 text-amber-600'}`}>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <Link 
                        ...
                        className="text-[10px] font-bold text-blue-600 hover:underline inline-flex items-center gap-0.5 mt-1"
                      >
          ...
          {quickAlerts.length > 3 && (
            <Link 
              href="/alerts"
              className="text-xs font-bold text-slate-500 hover:text-slate-700 flex items-center justify-between border-t border-slate-100 pt-3 mt-4"
            >
      ...
      <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-slate-100 text-slate-400 text-xs font-bold uppercase tracking-wider font-mono">
                  ...
                  <tr key={sale.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                    <td className="py-3">
                      <div className="leading-tight">
                        <span className="text-xs font-bold text-slate-700 block">{sale.external_id}</span>
                        <span className="text-[10px] text-slate-400 font-semibold uppercase">{sale.origen}</span>
                      </div>
                    </td>
                    <td className="py-3 text-xs font-semibold text-slate-600">{sale.sku}</td>
                    <td className="py-3 text-xs font-semibold text-slate-600">{sale.cantidad}</td>
                    <td className="py-3">
                      <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${sale.sae_decremented ? 'bg-green-50 text-green-700' : 'bg-slate-100 text-slate-500'}`}>
                        {sale.sae_decremented ? 'COMPLETO' : 'PENDIENTE'}
                      </span>
                    </td>
                    <td className="py-3">
                      <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${sale.shopify_synced ? 'bg-green-50 text-green-700' : 'bg-slate-100 text-slate-500'}`}>
                        {sale.shopify_synced ? 'COMPLETO' : 'PENDIENTE'}
                      </span>
                    </td>
                    <td className="py-3">
                      <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${sale.ml_synced ? 'bg-green-50 text-green-700' : 'bg-slate-100 text-slate-500'}`}>
